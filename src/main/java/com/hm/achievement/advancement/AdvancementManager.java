...
public class AdvancementManager {
	...
	private final Set<NamespacedKey> pluginAdvancements;
	private final boolean configRegisterAdvancementDescriptions;

	private int generatedAdvancements;

	public AdvancementManager(AdvancedAchievements plugin) {
		this.plugin = plugin;
		unsafeValues = Bukkit.getUnsafe();
		pluginAdvancements = new HashSet<>();
	...
	public void registerAdvancements() {
		...
		if (generatedAdvancements > 0) {
			plugin.getLogger().warning("New advancements were generated by Advanced Achievements. "
					+ "A full restart is required to allow the server to properly load them.");
		}
	}
	...
	private void registerParentAdvancement() {
		...
		pluginAdvancements.add(namespacedKey);
	...
	private String registerAdvancement(String categoryName, String configAchievement, String parentKey,
		...
		if (Bukkit.getServer().getAdvancement(namespacedKey) == null) {
			...
			AchievementAdvancementBuilder achievementAdvancementBuilder = new AchievementAdvancementBuilder()
					.iconItem("minecraft:"
							+ getInternalName(new ItemStack(getMaterial(categoryName), 1, (short) metadata)))
					.iconData(Integer.toString(metadata)).title(achDisplayName).description(description)
					.parent(parentKey);
			if (lastAchievement) {
	...
	/**
	 * Removes all advancements previously generated by the plugin, but that do no fit within the new configuration.
	 */
	private void cleanupOldAchievementAdvancements() {
		...
		while (advancements.hasNext()) {
			...
			if ("advancedachievements".equals(namespacedKey.getNamespace())
					&& !pluginAdvancements.contains(namespacedKey)) {
				plugin.getLogger().info("Cleaning old advancement " + namespacedKey + ".");
