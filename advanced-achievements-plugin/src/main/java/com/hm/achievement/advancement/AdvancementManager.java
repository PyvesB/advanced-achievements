...
/**
 * Class in charge of registering achievements as advancements for servers
 * running on Minecraft 1.12+.
 * 
 * @author Pyves
 */
...
public class AdvancementManager implements Reloadable {
	...
	public AdvancementManager(@Named("main") CommentedYamlConfiguration mainConfig,
			@Named("gui") CommentedYamlConfiguration guiConfig, AdvancedAchievements advancedAchievements,
			Logger logger, Map<String, List<Long>> sortedThresholds, Set<String> disabledCategories) {
	...
	/**
	 * Registers an "Advanced Achievements" advancement, which will be used as the
	 * parent of all advancements generated by Advanced Achievements.
	 */
	private void registerParentAdvancement() {
		...
		if (Bukkit.getServer().getAdvancement(namespacedKey) == null) {
			unsafeValues.loadAdvancement(namespacedKey, achievementAdvancementBuilder.buildGoal()
					.toParentJson(configHideAdvancements, configBackgroundTexture));
		}
	}
	...
	private void registerCategoryAdvancements(String categoryName, String subcategory) {
		...
		List<Long> orderedThresholds = subcategory.isEmpty()
				? sortedThresholds.get(categoryName)
	...
	/**
	 * Gets the internal item used by Vanilla Minecraft. These are the only names
	 * supported by advancements. Material and internal names can differ quite
	 * significantly (for instance: book_and_quill vs. writable_book).
	 * 
	 * @param item
